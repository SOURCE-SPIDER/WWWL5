...
from WWWL5 import Convert, WWWL5
...
@WWWL5.ar_cmd(pattern="(لصورة|تحويل صورة)$")
async def _(event):
...
@WWWL5.ar_cmd(pattern="(لملصق|تحويل ملصق)$")
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
...
@WWWL5.ar_cmd(pattern="الملف لصورة$")
...
@WWWL5.ar_cmd(pattern="(تحويل متحركة|لمتحركة)$")
async def _(event):
...
@WWWL5.ar_cmd(pattern="تحويل الى (صوتي|بصمة)")
async def _(event):
